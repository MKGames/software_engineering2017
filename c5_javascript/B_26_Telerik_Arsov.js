...
Spawner.prototype.spawn = function() {
	...
	//	setTimeout(function(){balls.push(new Ball(t.x, t.y, random(1, 5) + 0.5, random(-5, 5) + 0.5, 0.9, 50, 1, "scetch02.jpg", false))}, t.spawnInterval * (i + 1));
	//}	
	balls.push(new Ball(t.x + t.width / 2, t.y + t.height / 2, random(-5, 5) + 0.5, random(1, 5) + 0.5, 0.9, 50, 1, "scetch02.jpg", false));
}
...
function init() {
	for(var i = 0; i < 4; i++)
	{
		spawners.push(new Spawner(100 + i * 200, 50, 0, 0, 100, 100, spawnTime, numberOfBalls, "scetch02.jpg"));
...
function spawnBalls() {
	...
	setTimeout(function (){spawners[index].spawn();}, spawners[index].spawnInterval);
}
...
function draw() {
  	...
  	window.onkeydown = function(e) {
    	if(e.keyCode === 65) { // a
    		var r = random(50, 100)
			var ball = new Ball(random(0 + r, width - r), random(0 + r, height - r), random(-5, 5) + 0.5, random(-5, 5) + 0.5, 0.9, r, 10, "scetch02.jpg", false);
			balls.push(ball);
    	}
    	if(e.keyCode === 69) {// e
    		fillRectAlpha -= 1;
    		fillRectAlpha = Math.abs(fillRectAlpha);
    		console.log(fillRectAlpha);
    	}
    }
 	for(var i = 0; i < balls.length; i++) {  
 		if(!balls[i].dead) {
