...
results_file='./B_19_Peter_Mihaylov_results.csv'
...
CSV.foreach(students_file, headers: true, skip_blanks: true) do |row|
  Thread.new do
    ...
    if school_class.include?("Б") || school_class.include?("б") || school_class.include?("B") || school_class.include?("b")
      school_class = "B"
    elsif school_class.include?("А") || school_class.include?("а") || school_class.include?("A") || school_class.include?("a") 
      school_class = "A"
    end
    ...
    if heroku_url == nil
      ...
      sum_res = `curl -F "file=@B_19_Peter_Mihaylov.csv" #{heroku_url}sums 2>/dev/null`
      filters_res = `curl -F "file=@B_19_Peter_Mihaylov.csv" #{heroku_url}filters 2>/dev/null`
      intervals_res = `curl -F "file=@B_19_Peter_Mihaylov.csv" #{heroku_url}intervals 2>/dev/null`
      lin_reg_res = `curl -F "file=@B_19_Peter_Mihaylov.csv" #{heroku_url}lin_regressions 2>/dev/null`
